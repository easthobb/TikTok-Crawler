...
def parse_tik(page_source:str):
    ...
    for x in html.select('span.lazyload-wrapper'):
        ...
        if not content:
            continue
        content = content[0]
        video_id = str(content.select('a.item-video-card-wrapper')[0]['href']).split('/')[-1]
        ...
        reaction_params = {k['title']: k.text for k in content.select('div.pc-action-bar strong')}
...
def crawl_feed_info():
    ...
    #df.to_excel('tiktok_test_4.xlsx', index=False)
    return df
if __name__ == '__main__':
    
    df = crawl_feed_info()
    
    engine = create_engine("postgresql://hobbes:6132@localhost:5432/crawler")
    
    df.to_sql(name = 'feed_info_daily',
        con = engine,
        schema = 'public',
        if_exists = 'append',
        index = False,
